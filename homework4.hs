        ...
        then writeFile ouf (show parsed)
            ...
            in writeFile ouf $ case res of
                (Left (n, error)) -> "Вывод некорректен начиная с формулы № " ++ (show n) ++ case error of
                    ...
                    (BadUsageRule v e)      -> ": используется правило с квантором по переменной " ++ v ++ " входящей свободно в допущение"
                    (BadUsageScheme v e)    -> ": используется схема аксиом с квантором по переменной " ++ v ++ " входящей свободно в допущение"
                    _                       -> ""
                (Right f4)        -> show f4 ++ "\n"
