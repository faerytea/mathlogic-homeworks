...
import MathlogicParser --(MathlogicToken, PropVariable(..), )
...
-- toAnnotatedString :: Expression -> Integer -> AnnotatedString
-- toAnnotatedString e n = (e, "test", n, decompose e)

-- toAnnotatedList :: [Expression] -> [AnnotatedString]
-- toAnnotatedList exps = toAL exps 1 where
    -- toAL :: [Expression] -> Integer -> [AnnotatedString]
    -- toAL [] _     = []
    -- toAL (e:es) c = (toAnnotatedString e c):(toAL es (c+1))
